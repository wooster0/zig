//! Helper functions and structs for use within `test` blocks.
...
fn checkScope(maybe_src: ?std.builtin.SourceLocation) void {
    if (!builtin.is_test) {
        if (maybe_src) |src|
            @compileError(src.fn_name ++ " can only be used inside `test` blocks")
        else
